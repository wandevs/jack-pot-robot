  ...
  async chooseValidator() {
    ...
    for (let i=0; i<candidates.length; i++) {
      ...
      if (info) {
        ...
        if (client) {
          const amount = web3.utils.toBN(client.amount);
        }
      ...
      // all failed ? choose the biggest delegate out, choose the smallest to be the candidate
      if (i === candidates.length - 1) {
        ...
        if (validCandidates.length > 0) {
          log.info(`changing validator from ${validatorsInfo.currentValidator} to ${validCandidates[0].addr}`);
