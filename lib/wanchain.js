...
const signTx = (nonce, data, prvKey, value='0x00') => {
  const txParams = {
    Txtype: 0x01,
    nonce: nonce,
...
class JackPot {
  ...
  async doOperator(opName, data, value, count = 7) {
    ...
    const rawTx = signTx(nonce, data, process.env.JACKPOT_OPERATOR_PVKEY, value);
  ...
  async chooseValidator() {
    ...
    const blockNumber = await getBlocknumber();
    const stakersInfoOrigin = await pos.getStakerInfo(blockNumber);
  ...
  async stakeIn(codes, amounts) {
    ...
    const data = this.contract.methods.stakeIn(codes, amounts).encodeABI();
    return await this.doOperator(this.stakeIn.name, data, '0x' + total.toString('hex'));
  }

  async stakeOut(codes) {
    const data = this.contract.methods.stakeOut(codes).encodeABI();
    return await this.doOperator(this.stakeOut.name, data);
  }
}
