...
class JackPot {
  ...
  async chooseValidator() {
    ...
    if (validatorsInfo.validatorsCount <= 1) {
      return validatorsInfo.currentValidator;
    }
    ...
    // else if amount >= 20000, find one < 20000
    //    if find one, choose it
    //    else if all >= 20000,
    //        if can delegateOut choose one delegateOut
    // try again find one amount < 20000 * n, n=2,3,...
    ...
    if (candidateAddr === null) {
      candidateAddr = validatorsInfo.currentValidator;
    }
